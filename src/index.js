...
async function from3DS (imageBuffer) {
    ...
    let image = await jimp.read(buffer3DS);
    //let arr = [];
    for (let y = 0; y < 216; y++) {
        for (let x = 0; x < 168; x++) {
    ...
    image.quality(100).write(path.join(__dirname, '../image3ds.jpg'));
    
    buffer3DS = Buffer.concat([
    ...
    image = await jimp.read(buffer3DS);
    for (let y = 0; y < 240; y++) {
        for (let x = 0; x < 32; x++) {
    ...
    //arr.sort((a, b) => (a - b));
    //console.log(arr);
    image.quality(100).write(path.join(__dirname, '../image3ds_wide.jpg'));
}
...
(async () => {
    const course = fs.readFileSync(path.join(__dirname, '../course000'));
